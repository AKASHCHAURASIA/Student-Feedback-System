...
if(isset($_POST['yes']))
{
    ...
    $e = "cm_feed".date('My');
	$e1 = "time_table".date('My');
	$e2 = "ques".date('My');
	
         if(mysqli_query($conn, "CREATE TABLE `$e` SELECT * FROM cm_feed") && mysqli_query($conn, "CREATE TABLE `$e1` SELECT * FROM time_table") && mysqli_query($conn, "CREATE TABLE `$e2` SELECT * FROM ques")){
            
            if(mysqli_query($conn, "UPDATE users SET chk='0' WHERE chk=1")&& mysqli_query($conn, "TRUNCATE cm_feed")){
            if(mysqli_query($conn, "INSERT INTO previous_feed(f_date,year,month,branch, table_name,table_name1,table_name2) VALUES('$d','$y','$m','CM', '$e','$e1','$e2')")){   
...
{
	echo "<script>location.assign('subanalyzefeedback.php')</script>";
}
?>
