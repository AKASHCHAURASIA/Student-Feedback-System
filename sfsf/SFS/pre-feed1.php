...
function per($question,$teacher,$feed,$group1,$group2,$year)
{
	...
	$res1 = mysqli_query($conn, "SELECT * FROM users WHERE year='$year'  AND branch='$branch'");
	...
	else
	{
			...
			print" <div class='col-lg-12'><h1 class='page-header' style='font-size: 18px;'>".$batch2."".$batch3."  SEMESTER-".$sem."</h1></div>";
				print" <div class='col-lg-12'><h1 class='page-header' style='font-size: 18px;'>Previous feedback report:-".$batch4."</h1></div>";
	
	
	print" <h1 class='page-header' style='font-size: 18px;'>".$msg."</h1>";
	echo "<table border='2' style='width:90%' height:100%'>";
	$i = $j = 0;
	$sum = 0;
	$total = array();
	$value = array();
